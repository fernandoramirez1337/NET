...
board server_interface::draw_board() {
    board new_board(HEIGHT, std::vector<char>(WIDTH, ' ')); // Initialize the board with spaces

    for (auto& snakee : snakes) {
        if (!snakee.second.game_over) {
            for (auto& segment : snakee.second.body) {
                int x = segment.x;
                int y = segment.y;

                if (x >= 0 && x < WIDTH && y >= 0 && y < HEIGHT) { // Boundary check
                    new_board[y][x] = snakee.first; // Update the board
                }
            }
        }
    }
