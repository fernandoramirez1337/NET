...
udp_client::udp_client(char * port , char * username){
    ...
    std::cout << "Waiting for messages in port " << port << std::endl;
}
...
void * udp_client::receive(){
    ...
    while (1){
        ...
        std::cout << "(" << received.username <<"@"<< inet_ntoa(tmp_addr.sin_addr) << ":" << ntohs(tmp_addr.sin_port) << ") said: " << received.payload << std::endl;
    }
}
...
void udp_client::send(){
    ...
    do{
        ...
        if(sendto(sock, formatted_buffer.c_str(), MAXDATASIZE, 0,(struct sockaddr *)&receiver_addr, sizeof(struct sockaddr)) == -1)
            perror("sendto");
    }   while(buffer != ".exit");
}
